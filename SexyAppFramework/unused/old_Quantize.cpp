...
static unsigned int Assignment(CubeInfo* cube_info, PixelPacket* thePixelPackets, int theWidth, int theHeight,
                               uchar* theIndices, PixelPacket* theColorTable)
{
    uchar index;
    long  count, y;
...
void Sexy::Quantize8Bit(const ulong* theSrcBits, int theWidth, int theHeight, uchar* theDestColorIndices,
                        ulong* theDestColorTable)
{
    ...
    if (status != false)
    {
        ...
        aThing = timeGetTime();
        // 296 ms
        Assignment(cube_info, (PixelPacket*)theSrcBits, theWidth, theHeight, (uchar*)theDestColorIndices,
...
void Sexy::Quantize8Bit2(const ulong* theSrcBits, int theWidth, int theHeight, uchar* theDestColorIndices,
                         ulong* theDestColorTable)
{
    int i;

    for (i = 0; i < 512; i++) gSignedPowerTable[i] = (i - 256) * (i - 256);

    for (i = 0; i < HASH_TABLE_SIZE; i++) gColorRecordsHashTable[i] = NULL;
