...
int memory_output_module_query(XA_OutputModuleQuery* query, unsigned long query_mask)
{
    return XA_SUCCESS;
}

int memory_output_new(XA_OutputInstance** output, const char* name, XA_OutputModuleClassInfo* class_info,
...
int memory_output_write(XA_OutputInstance* output, const void* buffer, unsigned long size,
                        unsigned int bytes_per_sample, unsigned int nb_channels, unsigned int sample_rate)
{
    MemoryOutput* aMemoryOutput = (MemoryOutput*)output;
    ...
    aMemoryOutput->mByteVector.insert(aMemoryOutput->mByteVector.end(), (const unsigned char*)buffer,
