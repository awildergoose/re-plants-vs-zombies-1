...
static const char* FindFileInStringTable(const std::string& theSearch, const char* theMem, DWORD theLength,
...
static bool ModStringToString(const char* theString, std::string& theStrVal)
{
    ...
    while (true)
    {
        ...
        {
            i++;
            while (isspace((unsigned char)theString[i])) i++;
...
bool Sexy::ReparseModValues()
{
    ...
    for (aFileModItr = aMap.begin(); aFileModItr != aMap.end(); ++aFileModItr)
    {
            ...
            while (!aStream.eof())
            {
                ...
                while (aString[aCharIdx] != 0)
                {
                    ...
                    else if (aChar == '(')
                    {
                        ...
                        if (theAreaNum != -1)
                        {
                            while (aModMapItr != aModMap.end() && aModMapItr->second.mLineNum < aLineNum) ++aModMapItr;
        ...
        {
            MessageBoxA(NULL, (std::string("ERROR: Unable to open ") + aFileName + " for reparsing.").c_str(),
                        "MODVAL ERROR!", MB_OK | MB_ICONERROR);
            return false;
        }
    }
