...
struct
{
    ...
    char* szMessage;
} gMsgTable[] = {{STATUS_SEGMENT_NOTIFICATION, (char*)"Segment Notification"},
...
bool SEHCatcher::LoadImageHelp()
{
    ...
    mSymInitialize = SexyGetProc<SYMINITIALIZEPROC>(mImageHelpLib, "SymInitialize");
    if (!mSymInitialize) return false;

    mSymSetOptions = SexyGetProc<SYMSETOPTIONSPROC>(mImageHelpLib, "SymSetOptions");
    if (!mSymSetOptions) return false;

    mSymCleanup = SexyGetProc<SYMCLEANUPPROC>(mImageHelpLib, "SymCleanup");
    if (!mSymCleanup) return false;

    mUnDecorateSymbolName = SexyGetProc<UNDECORATESYMBOLNAMEPROC>(mImageHelpLib, "UnDecorateSymbolName");
    if (!mUnDecorateSymbolName) return false;

    mStackWalk = SexyGetProc<STACKWALKPROC>(mImageHelpLib, "StackWalk");
    if (!mStackWalk) return false;

    mSymFunctionTableAccess = SexyGetProc<SYMFUNCTIONTABLEACCESSPROC>(mImageHelpLib, "SymFunctionTableAccess");
    if (!mSymFunctionTableAccess) return false;

    mSymGetModuleBase = SexyGetProc<SYMGETMODULEBASEPROC>(mImageHelpLib, "SymGetModuleBase");
    if (!mSymGetModuleBase) return false;

    mSymGetSymFromAddr = SexyGetProc<SYMGETSYMFROMADDRPROC>(mImageHelpLib, "SymGetSymFromAddr");
...
std::string SEHCatcher::ImageHelpWalk(PCONTEXT theContext, int theSkipCount)
{
    ...
    for (;;)
    {
        ...
        if (mSymGetSymFromAddr(GetCurrentProcess(), sf.AddrPC.Offset, &symDisplacement, pSymbol))
        {
            ...
            sprintf(aBuffer, "%016llX %016llX %s+%lX\r\n", sf.AddrFrame.Offset, sf.AddrPC.Offset, aUDName,
                    symDisplacement);
        }
        else  // No symbol found.  Print out the logical address instead.
        {
            ...
            sprintf(aBuffer, "%016llX %016llX %08lX:%08lX %s\r\n", sf.AddrFrame.Offset, sf.AddrPC.Offset, section,
                    offset, GetFilename(szModule).c_str());
        }
        aDebugDump += aBuffer;
    ...
    switch (uMsg)
    {
        ...
        case WM_TIMER:
        {
            if (wParam == 0)
            {
                ...
                if ((aResult != HTTPTransfer::RESULT_NOT_COMPLETED) && (aResult != HTTPTransfer::RESULT_NOT_STARTED))
                {
                    if ((aResult != HTTPTransfer::RESULT_DONE) && (aResult != HTTPTransfer::RESULT_ABORTED))
                    {
                        ...
                        if (::MessageBoxW(hWnd, SEHCatcher::mSubmitErrorMessage.c_str(), L"Connection Failed",
            ...
            else if (wParam == 1)
            {
                ...
                for (int i = 0; i < (aCount % 10); i++) aNewString += " .";
...
static void CreateProgressWindow()
{
    ...
    HWND aHWnd = CreateWindowA("SEHProgressWindow", "Submitting Report", aWindowStyle, 64, 64, aRect.right - aRect.left,
    ...
    gEditWindow =
        CreateWindowA("EDIT", "Please Wait",
    ...
    SEHCatcher::mNoButtonWindow =
        CreateWindowA("BUTTON", "Abort", WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON | BS_PUSHBUTTON, (240 - 96) / 2,
                      64 - 22 - 6, 96, 22, aHWnd, NULL, gHInstance, 0);
...
void SEHCatcher::ShowSubmitInfoDialog()
{
    ...
    HWND aHWnd = CreateWindowA("SubmitInfoWindow", "Error Details", aWindowStyle, 64 + 16, 64 + 16,
                               aRect.right - aRect.left, aRect.bottom - aRect.top, NULL, NULL, gHInstance, 0);

    HWND aLabelWindow = CreateWindowW(L"EDIT", mSubmitMessage.c_str(),

                                      WS_VISIBLE | WS_CHILD | ES_MULTILINE | ES_READONLY, 8, 8, 400 - 8 - 8, 84, aHWnd,
                                      NULL, gHInstance, 0);

    HDC aDC         = ::GetDC(aLabelWindow);
    int aFontHeight = -MulDiv(9, 96, 72);
    ::ReleaseDC(aLabelWindow, aDC);
    HFONT aBoldArialFont = CreateFontA(aFontHeight, 0, 0, 0, FW_BOLD, 0, 0, false, ANSI_CHARSET, OUT_DEFAULT_PRECIS,
    ...
    mEditWindow = CreateWindowA("EDIT", "", WS_VISIBLE | WS_CHILD | ES_MULTILINE | WS_BORDER | WS_VSCROLL, 8,
    ...
    HFONT aCourierNewFont =
        CreateFontA(aFontHeight, 0, 0, 0, FW_NORMAL, 0, 0, false, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
    ...
    mYesButtonWindow = CreateWindowA("BUTTON", "Continue", aWindowStyle, aCurX, 300 - 24 - 8, aButtonWidth, 24, aHWnd,
                                     NULL, gHInstance, 0);
    if (!gUseDefaultFonts) SendMessage(mYesButtonWindow, WM_SETFONT, (WPARAM)aBoldArialFont, 0);

    aCurX += aButtonWidth + 8;

    mNoButtonWindow = CreateWindowA("BUTTON", "Abort", WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON | BS_PUSHBUTTON, aCurX,
                                    300 - 24 - 8, aButtonWidth, 24, aHWnd, NULL, gHInstance, 0);
...
void SEHCatcher::ShowErrorDialog(const std::string& theErrorTitle, const std::string& theErrorText)
{
    ...
    GetVersionExModern(&aVersionInfo);
    ...
    mDialogFont = ::CreateFontA(aHeight, 0, 0, 0, FW_NORMAL, FALSE, FALSE, false, ANSI_CHARSET, OUT_DEFAULT_PRECIS,
                                CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, "Tahoma");

    aHeight   = -MulDiv(10, 96, 72);
    mBoldFont = ::CreateFontA(aHeight, 0, 0, 0, FW_BOLD, FALSE, FALSE, false, ANSI_CHARSET, OUT_DEFAULT_PRECIS,
                              CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, "Tahoma");
    ...
    HWND aHWnd = CreateWindowW(L"SEHWindow", L"Fatal Error!", aWindowStyle, 64, 64, aRect.right - aRect.left,
                               aRect.bottom - aRect.top, NULL, NULL, gHInstance, 0);

    HWND aLabelWindow =
        CreateWindowW(L"EDIT", mCrashMessage.c_str(), WS_VISIBLE | WS_CHILD | ES_MULTILINE | ES_READONLY, 8, 8,
                      400 - 8 - 8, 84, aHWnd, NULL, gHInstance, 0);
    ...
    HWND anEditWindow = CreateWindowA("EDIT", theErrorText.c_str(),
                                      WS_VISIBLE | WS_CHILD | ES_MULTILINE | WS_BORDER | WS_VSCROLL | ES_READONLY, 8,
                                      300 - 168 - 24 - 8 - 8, 400 - 8 - 8, 168, aHWnd, NULL, gHInstance, 0);
    ...
    if (canSubmit)
    {
        mYesButtonWindow = CreateWindowA("BUTTON", "Send Report", aWindowStyle, aCurX, 300 - 24 - 8, aButtonWidth, 24,
    ...
    if (doDebugButton)
    {
        mDebugButtonWindow = CreateWindowA("BUTTON", "Debug", aWindowStyle, aCurX, 300 - 24 - 8, aButtonWidth, 24,
                                           aHWnd, NULL, gHInstance, 0);
    ...
    mNoButtonWindow = CreateWindowA("BUTTON", "Close Now", WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON | BS_PUSHBUTTON,
                                    aCurX, 300 - 24 - 8, aButtonWidth, 24, aHWnd, NULL, gHInstance, 0);
