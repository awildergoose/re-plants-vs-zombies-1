...
void res0_free_look(vorbis_look_residue* i)
{
    ...
    if (i)
    {
        ...
        for (j = 0; j < look->partvals; j++) _ogg_free(look->decodemap[j]);
...
vorbis_info_residue* res0_unpack(vorbis_info* vi, oggpack_buffer* opb)
{
    ...
    for (j = 0; j < acc; j++) info->booklist[j] = oggpack_read(opb, 8);
...
static int _01inverse(vorbis_block* vb, vorbis_look_residue* vl, ogg_int32_t** in, int ch,
                      long (*decodepart)(codebook*, ogg_int32_t*, oggpack_buffer*, int, int))
{
    ...
    for (j = 0; j < ch; j++) partword[j] = (int**)_vorbis_block_alloc(vb, partwords * sizeof(*partword[j]));
        ...
        for (i = 0, l = 0; i < partvals; l++)
        {
            ...
            for (k = 0; k < partitions_per_word && i < partvals; k++, i++)
                if (info->secondstages[partword[l][k]] & (1 << s))
                {
                    ...
                    if (stagebook)
                    {
                        if (vorbis_book_decodevv_add(stagebook, in, i * samples_per_partition + beginoff, ch, &vb->opb,
