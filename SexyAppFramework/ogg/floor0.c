...
void vorbis_lsp_to_curve(ogg_int32_t* curve, int* map, int n, ogg_int32_t* lsp, int m, ogg_int32_t amp,
                         ogg_int32_t ampoffset, ogg_int32_t* icos)
{
    /* 0 <= m < 256 */
...
static vorbis_look_floor* floor0_look(vorbis_dsp_state* vd, vorbis_info_mode* mi, vorbis_info_floor* i)
{
    ...
    for (j = 0; j < look->ln; j++) look->lsp_look[j] = vorbis_coslook2_i(0x10000 * j / look->ln);
