...
// ====================================================================================================
// ★ 一个 PakRecord
// 实例对应资源包内的一个资源文件的数据，包括文件名，地址，大小等信息
// ====================================================================================================
class PakRecord
{
   public:
    PakCollection* mCollection;  //+0x0：指向该资源文件所在的资源包的 PakCollection
    std::string    mFileName;    //+0x4：资源文件的名称及路径（路径从 .pak 开始），例如
                                 // sounds\zombie_falling_1.ogg
    FILETIME mFileTime;          //+0x20：八字节型的资源文件的时间戳
    int      mStartPos;          //+0x28：该资源文件在资源包中的位置（即在 mCollection->mDataPtr
                                 // 中的偏移量）
...
class PakInterfaceBase
{
   public:
    ...
    //	virtual PFILE*			FOpen(const wchar_t* theFileName, const
    // wchar_t* theAccess) { return NULL; }
    ...
    //	virtual wchar_t*		FGetS(wchar_t* thePtr, int theSize,
    // PFILE* theFile) { return thePtr; }
...
{
   public:
    PakCollectionList mPakCollectionList;  //+0x4：通过 AddPakFile()
                                           // 添加的各个资源包的内存映射文件数据的链表
