...
SexyString ListWidget::GetSortKey(int theIdx)
{
    ...
    while (aString.length() < (ulong)mMaxNumericPlaces) aString = _S("0") + aString;
...
void ListWidget::Sort(bool ascending)
{
    ...
    while (aListWidget->mParent != NULL) aListWidget = aListWidget->mParent;
...
int ListWidget::GetOptimalWidth()
{
    ...
    for (ulong i = 0; i < mLines.size(); i++) aMaxWidth = std::max(aMaxWidth, mFont->StringWidth(mLines[i]));
