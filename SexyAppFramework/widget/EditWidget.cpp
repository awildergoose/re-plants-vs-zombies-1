...
void EditWidget::EnforceMaxPixels()
{
    ...
    {
        while (mFont->StringWidth(mString) > mMaxPixels) mString = mString.substr(0, mString.length() - 1);
    ...
    for (WidthCheckList::iterator anItr = mWidthCheckList.begin(); anItr != mWidthCheckList.end(); ++anItr)
    {
        ...
        while (anItr->mFont->StringWidth(mString) > aWidth) mString = mString.substr(0, mString.length() - 1);
    }
}
...
void EditWidget::ProcessKey(KeyCode theKey, SexyChar theChar)
{
    ...
    else if (theKey == KEYCODE_LEFT)
    {
        ...
        {
            // Get to a word
            while ((mCursorPos > 0) && (!IsPartOfWord(mString[mCursorPos - 1]))) mCursorPos--;
    ...
    else if (theKey == KEYCODE_RIGHT)
    {
        ...
        {
            // Get to whitespace
            while ((mCursorPos < (int)mString.length() - 1) && (IsPartOfWord(mString[mCursorPos + 1]))) mCursorPos++;
...
void EditWidget::HiliteWord()
{
    ...
    if (mCursorPos < (int)aString.length())
    {
        ...
        while ((mHilitePos > 0) && (IsPartOfWord(aString[mHilitePos - 1]))) mHilitePos--;

        // Find first space after word
        while ((mCursorPos < (int)aString.length() - 1) && (IsPartOfWord(aString[mCursorPos + 1]))) mCursorPos++;
