...
SexyAppBase::SexyAppBase()
{
    ...
    gGetLastInputInfoFunc = SexyGetProc<GetLastInputInfoFunc>(GetModuleHandleA("user32.dll"), "GetLastInputInfo");
    ...
    for (i = 0; i < NUM_CURSORS; i++) mCursorImages[i] = NULL;

    for (i = 0; i < 256; i++) mAdd8BitMaxTable[i] = i;

    for (i = 256; i < 512; i++) mAdd8BitMaxTable[i] = 255;
...
void SexyAppBase::DemoSyncBuffer(Buffer* theBuffer)
{
    if (mPlayingDemoBuffer)
    {
        ...
        theBuffer->Clear();
        for (int i = 0; i < (int)aLen; i++) theBuffer->WriteByte(mDemoBuffer.ReadByte());
    }
...
std::string SexyAppBase::GetProductVersion(const std::string& thePath)
{
    ...
    {
        aGetFileVersionInfoSizeFunc = SexyGetProc<GetFileVersionInfoSizeFunc>(gVersionDLL, "GetFileVersionInfoSizeA");
        aGetFileVersionInfoFunc     = SexyGetProc<GetFileVersionInfoFunc>(gVersionDLL, "GetFileVersionInfoA");
        aVerQueryValueFunc          = SexyGetProc<VerQueryValueFunc>(gVersionDLL, "VerQueryValueA");
    }
...
{
    while ((mLoadingThreadStarted) && (!mLoadingThreadCompleted)) Sleep(20);
}
...
void SexyAppBase::DumpProgramInfo()
{
    ...
    for (;;)
    {
        if (_mkdir((GetAppDataFolder() + "_dump").c_str())) break;
        Sleep(100);
    }
...
bool SexyAppBase::RegistryGetSubKeys(const std::string& theKeyName, StringVector* theSubKeys)
{
    ...
    if (mPlayingDemoBuffer)
    {
        ...
        for (int i = 0; i < aNumKeys; i++) theSubKeys->push_back(mDemoBuffer.ReadString());
    ...
    else
    {
        ...
        if (aResult == ERROR_SUCCESS)
        {
            ...
            if (mRecordingDemoBuffer)
            {
                ...
                for (int i = 0; i < (int)theSubKeys->size(); i++) mDemoBuffer.WriteString((*theSubKeys)[i]);
            }
                                  ...
                                  HKEY theKey)
{
    ...
    if (mPlayingDemoBuffer)
    {
        ...
        *theLength = aLen;
        ...
        {
            for (int i = 0; i < (int)aLen; i++) mDemoBuffer.ReadByte();
            return false;
        }
    }
    else
...
bool SexyAppBase::DrawDirtyStuff()
{
        ...
        ((int)(aStartTime - mNextDrawTick) >= 0))
    {
        ...
        mFPSTime)).c_str()); OutputDebugString(StrFormat(_S("Actual      FPS: %d\r\n"), (mFPSFlipCount * 1000) /
        max((aTickNow - mFPSStartTick), 1)).c_str()); OutputDebugString(StrFormat(_S("Dirty Rate     : %d\r\n"),
...
void SexyAppBase::Popup(const std::wstring& theString)
{
    ...
    BeginPopup();
    if (!mShutdown)
        ::MessageBoxW(mHWnd, theString.c_str(),
    ...
    switch (msg)
    {
        case WM_INITDIALOG:
        {
                 ...
                 anItr != gSexyAppBase->mDemoMarkerList.end(); ++anItr)
            {
                ...
                SexyString aStr =
                    StrFormat(_S("%s (%02d:%02d:%02d)"), anItr->first.c_str(), anHours, aMinutes, aSeconds);
            ...
            if (aVersion.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS)
            {
                ...
                if (RegOpenKey(HKEY_CURRENT_USER, REGSTR_PATH_SCREENSAVE, &hKey) == ERROR_SUCCESS)
                {
                    ...
                    if (aResult == ERROR_SUCCESS && aCheckPwd)
                    {
                        ...
                        if (aPasswordLib)
                        {
                            aPasswordFunc = SexyGetProc<VERIFYPWDPROC>(aPasswordLib, "VerifyScreenSavePwd");
...
LRESULT CALLBACK SexyAppBase::WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        ...
        {
            /*			if (aSexyApp!=NULL && aSexyApp->mProcessInTimer && !aSexyApp->mShutdown &&
               aSexyApp->mRunning)
                                    {
            ...
            if ((aSexyApp != NULL) && (!aSexyApp->mNoDefer))
            {
                ...
                */
                    ...
                    if (uMsg == WM_CHAR)
                    {
                        switch (wParam)
                        {
                            ...
                            case '4':
                                ...
                                aSexyApp->mFastForwardToUpdateNum =
                                    aSexyApp->mDemoLength - (120000 / aSexyApp->mFrameTime);
                                break;
                            case '5':
                                // Fast foward to 90 seconds before it ends
                                aSexyApp->mFastForwardToUpdateNum =
                                    aSexyApp->mDemoLength - (90000 / aSexyApp->mFrameTime);
                                break;
                            case '6':
                                // Fast foward to 60 seconds before it ends
                                aSexyApp->mFastForwardToUpdateNum =
                                    aSexyApp->mDemoLength - (60000 / aSexyApp->mFrameTime);
                                break;
                            case '7':
                                // Fast foward to 30 seconds before it ends
                                aSexyApp->mFastForwardToUpdateNum =
                                    aSexyApp->mDemoLength - (30000 / aSexyApp->mFrameTime);
                                break;
                            case '8':
                                // Fast foward to 10 seconds before it ends
                                aSexyApp->mFastForwardToUpdateNum =
                                    aSexyApp->mDemoLength - (10000 / aSexyApp->mFrameTime);
                                break;
                            case '9':
                                // Fast foward to 5 seconds before it ends
                                aSexyApp->mFastForwardToUpdateNum =
                                    aSexyApp->mDemoLength - (5000 / aSexyApp->mFrameTime);
                                break;
                            case '0':
                ...
                if (aSexyApp->mDeferredMessages.size() > 0)
                {
                    ...
                    if (pushMessage && (uMsg == WM_SYSCOLORCHANGE ||
                                        uMsg == WM_DISPLAYCHANGE))  // kill duplicate SysColorChange() events.
                    {
        ...
        case WM_SYSCOMMAND:
            ...
            /*	case WM_DISPLAYCHANGE:
                            ...
                            if (aSexyApp!=NULL && aSexyApp->mIsWindowed && aSexyApp->mDDInterface!=NULL &&
               aSexyApp->mHWnd==hWnd && aSexyApp->mLoaded)
                            {
...
void SexyAppBase::ClearKeysDown()
{
    ...
    {
        for (int aKeyNum = 0; aKeyNum < 0xFF; aKeyNum++) mWidgetManager->mKeyDown[aKeyNum] = false;
    }
    mCtrlDown = false;
    mAltDown  = false;
}
            ...
            if (PrepareDemoCommand(false))
            {
                ...
                else
                {
                    switch (mDemoCmdNum)
                    {
                        ...
                        {
                            int sizeMult =
                                (int)mDemoBuffer.ReadNumBits(1, false) + 1;  // will be 1 for single, 2 for double
        ...
        if (!mPlayingDemoBuffer)
        {
            switch (uMsg)
            {
                ...
                case WM_SYSKEYDOWN:
                    ...
                    else if ((wParam == 'D') && (mWidgetManager != NULL) &&
                             (mWidgetManager->mKeyDown[KEYCODE_CONTROL]) && (mWidgetManager->mKeyDown[KEYCODE_MENU]))
                    {
                        PlaySoundA("c:\\windows\\media\\Windows XP Menu Command.wav", NULL, SND_ASYNC);
...
void SexyAppBase::MakeWindow()
{
    ...
    if ((mPlayingDemoBuffer) || (mIsWindowed && !mFullScreenWindow))
    {
        ...
        mHWnd = CreateWindowEx(0, _S("MainWindow"), SexyStringToStringFast(mTitle).c_str(), aWindowStyle, aPlaceX,
                               aPlaceY, aWidth, aHeight, NULL, NULL, gHInstance, 0);

        if (mPreferredX == -1)
        {
            ::MoveWindow(mHWnd, aDesktopRect.left + ((aDesktopRect.right - aDesktopRect.left) - aWidth) / 2,
                         aDesktopRect.top + (int)(((aDesktopRect.bottom - aDesktopRect.top) - aHeight) * 0.382), aWidth,
                         aHeight, FALSE);
        }
    ...
    {
        mHWnd = CreateWindowEx(WS_EX_TOPMOST, _S("MainWindow"), SexyStringToStringFast(mTitle).c_str(),
                               WS_POPUP | WS_VISIBLE, 0, 0, mWidth, mHeight, NULL, NULL, gHInstance, 0);
...
void SexyAppBase::Init()
{
    ...
    if (gDDrawDLL == NULL || gDSoundDLL == NULL)
    {
        MessageBox(NULL,
                   GetString("APP_REQUIRES_DIRECTX", _S("This application requires DirectX to run.  You can get "
                                                        "DirectX at http://www.microsoft.com/directx"))
                       .c_str(),
    ...
    // Change directory
    if (!ChangeDirHook(mChangeDirTo.c_str())) _chdir(mChangeDirTo.c_str());
    ...
    */
    ...
    mDraggingCursor = CreateCursor(gHInstance, 15, 10, 32, 32, gDraggingCursorData,
...
SharedImageRef SexyAppBase::SetSharedImage(const std::string& theFileName, const std::string& theVariant,
                                           DDImage* theImage, bool* isNew)
{
