...
Image* GetPNGImage(const std::string& theFileName)
{
    ...
    {
        row_pointers[i] = reinterpret_cast<png_bytep>(aBits + i * width);
    }
    ...
    */
    ...
    if (BitSet(flag, 0x80))  // 如果存在全局颜色列表
    {
        ...
        */
        global_colors =
            1 << ((flag & 0x07) +
                  1);  // 压缩字节的最低 3 位表示全局颜色列表的大小，设其二进制数值为 N，则列表大小 = 2 ^ (N + 1)
...
bool ImageLib::WriteBMPImage(const std::string& theFileName, Image* theImage)
{
    ...
    for (int i = 0; i < theImage->mHeight; i++, aRow -= theImage->mWidth) fwrite(aRow, 4, theImage->mWidth, aFile);
