...
void Graphics::DrawRect(int theX, int theY, int theWidth, int theHeight)
{
    ...
    else
    {
        ...
        /*if (aClippedRect.mX == aDestRect.mX)
                mDestImage->FillRect(Rect(aClippedRect.mX, aClippedRect.mY, 1, aClippedRect.mHeight), mColor,
        mDrawMode); if (aClippedRect.mY == aDestRect.mY) mDestImage->FillRect(Rect(aClippedRect.mX, aClippedRect.mY,
        aClippedRect.mWidth, 1), mColor, mDrawMode); if (aClippedRect.mX + aClippedRect.mWidth == aDestRect.mX +
        aDestRect.mWidth) mDestImage->FillRect(Rect(aClippedRect.mX + aClippedRect.mWidth, aClippedRect.mY, 1,
        aClippedRect.mHeight), mColor, mDrawMode); if (aClippedRect.mY + aClippedRect.mHeight == aDestRect.mY +
        aDestRect.mHeight) mDestImage->FillRect(Rect(aClippedRect.mX, aClippedRect.mY + aClippedRect.mHeight,
...
void Graphics::PolyFill(const Point* theVertexList, int theNumVertices, bool convex)
{
    ...
    for (k = 0; k < mPFNumVertices; k++) ind[k] = k;
...
int Graphics::WriteString(const SexyString& theString, int theX, int theY, int theWidth, int theJustification,
...
static int WriteWordWrappedHelper(Graphics* g, const SexyString& theString, int theX, int theY, int theWidth,
    ...
    while (aCurPos < theLine.length())
    {
        ...
        if (aCurWidth > theRect.mWidth)  // need to wrap
        {
            ...
            if (aSpacePos != -1)
            {
                ...
                if (aCurChar != _S('\n'))
                {
                    while (aCurPos < theLine.length() && theLine[aCurPos] == _S(' ')) aCurPos++;
                }
...
int Graphics::DrawStringWordWrapped(const SexyString& theLine, int theX, int theY, int theWrapWidth, int theLineSpacing,
                                    int theJustification, int* theMaxWidth)
{
