...
ulong* MemoryImage::GetBits()
{
    if (mBits == NULL)
    {
        ...
        if (mColorTable != NULL)
        {
            for (int i = 0; i < aSize; i++) mBits[i] = mColorTable[mColorIndices[i]];
...
void MemoryImage::FillRect(const Rect& theRect, const Color& theColor, int theDrawMode)
{
    ...
    if (oldAlpha == 0xFF)
    {
        for (int aRow = theRect.mY; aRow < theRect.mY + theRect.mHeight; aRow++)
        {
            ...
            for (int i = 0; i < theRect.mWidth; i++) *aDestPixels++ = src;
        }
    }
    else
    {
...
void MemoryImage::ClearRect(const Rect& theRect)
{
    ...
    for (int aRow = theRect.mY; aRow < theRect.mY + theRect.mHeight; aRow++)
    {
        ...
        for (int i = 0; i < theRect.mWidth; i++) *aDestPixels++ = 0;
    }
...
void MemoryImage::Clear()
{
    ...
    if (ptr != NULL)
    {
        for (int i = 0; i < mWidth * mHeight; i++) *ptr++ = 0;
                                  ...
                                  bool blend)
{
    ...
    SWHelper::XYZStruct aVerts[4] = {{-w2, -h2, u0, v0, static_cast<long int>(0xFFFFFFFF)},
...
void MemoryImage::BltMatrix(Image* theImage, float x, float y, const SexyMatrix3& theMatrix, const Rect& theClipRect,
