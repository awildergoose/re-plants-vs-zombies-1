...
bool DDImage::PolyFill3D(const Point theVertices[], int theNumVertices, const Rect* theClipRect, const Color& theColor,
...
void DDImage::AdditiveDrawLine(double theStartX, double theStartY, double theEndX, double theEndY,
...
void DDImage::DrawLine(double theStartX, double theStartY, double theEndX, double theEndY, const Color& theColor,
                       int theDrawMode)
{
    if (Check3D(this))
    {
...
void DDImage::NormalDrawLineAA(double theStartX, double theStartY, double theEndX, double theEndY,
                               const Color& theColor)
{
    LPDIRECTDRAWSURFACE aSurface = GetSurface();
    (void)aSurface;  // unused
...
void DDImage::DrawLineAA(double theStartX, double theStartY, double theEndX, double theEndY, const Color& theColor,
                         int theDrawMode)
{
    if (Check3D(this))
    {
    ...
    if (mLockedSurfaceDesc.ddpfPixelFormat.dwRGBBitCount == 16)
    {
        if (theColor.mAlpha == 255)
        {
            ...
            for (int y = 0; y < theRect.mHeight; y++)
            {
                ...
                for (int x = 0; x < theRect.mWidth; x++) *(aDestPixels++) = aColor;
...
void DDImage::NormalBlt(Image* theImage, int theX, int theY, const Rect& theSrcRect, const Color& theColor)
{
    ...
    if (aMemoryImage != NULL)
    {
        ...
        RECT aSrcRect  = {theSrcRect.mX, theSrcRect.mY, theSrcRect.mX + theSrcRect.mWidth,
                          theSrcRect.mY + theSrcRect.mHeight};
...
void DDImage::BltMirror(Image* theImage, int theX, int theY, const Rect& theSrcRect, const Color& theColor,
                        int theDrawMode)
{
    DBG_ASSERTE((theColor.mRed >= 0) && (theColor.mRed <= 255));
    DBG_ASSERTE((theColor.mGreen >= 0) && (theColor.mGreen <= 255));
...
void DDImage::BltF(Image* theImage, float theX, float theY, const Rect& theSrcRect, const Rect& theClipRect,
    ...
    if (Check3D(this))
    {
        ...
        if (anIntersect.mWidth != aDestRect.mWidth || anIntersect.mHeight != aDestRect.mHeight)
        {
            if (anIntersect.mWidth != 0 && anIntersect.mHeight != 0)
                mDDInterface->mD3DInterface->BltClipF(theImage, theX, theY, theSrcRect, &theClipRect, theColor,
                                                      theDrawMode);
        }
        else
...
// 0x57A610
void DDImage::BltRotated(Image* theImage, float theX, float theY, const Rect& theSrcRect, const Rect& theClipRect,
                         ...
                         float theRotCenterY)
{
    ...
    if (Check3D(this))
    {
        mDDInterface->mD3DInterface->BltRotated(theImage, theX, theY, &theClipRect, theColor, theDrawMode, theRot,
    ...
    if ((mDrawToBits) || (mHasAlpha) || ((mHasTrans) && (!mFirstPixelTrans)) || (mDDInterface->mIs3D))
    {
        MemoryImage::BltRotated(theImage, theX, theY, theSrcRect, theClipRect, theColor, theDrawMode, theRot,
...
void DDImage::StretchBlt(Image* theImage, const Rect& theDestRectOrig, const Rect& theSrcRectOrig,
                         const Rect& theClipRect, const Color& theColor, int theDrawMode, bool fastStretch)
{
    ...
    if (Check3D(this))
    {
        mDDInterface->mD3DInterface->StretchBlt(theImage, theDestRectOrig, theSrcRectOrig, &theClipRect, theColor,
    ...
    if (fastStretch)
    {
            ...
            (!aSrcDDImage->mHasAlpha) && (aSrcDDImage->GetSurface() != NULL))
        {
            ...
            RECT aDestRect = {theDestRect.mX, theDestRect.mY, theDestRect.mX + theDestRect.mWidth,
                              theDestRect.mY + theDestRect.mHeight};
            RECT aSrcRect  = {(LONG)theSrcRect.mX, (LONG)theSrcRect.mY, (LONG)(theSrcRect.mX + theSrcRect.mWidth),
...
#undef _PLUSPLUS
    ...
    else
    {
        if ((mDrawToBits) || (mHasAlpha) || (mHasTrans) || (mDDInterface->mIs3D))
        {
            MemoryImage::StretchBlt(theImage, theDestRectOrig, theSrcRectOrig, theClipRect, theColor, theDrawMode,
                                    fastStretch);
            return;
        }
...
// 0x583AF0
void DDImage::StretchBltMirror(Image* theImage, const Rect& theDestRectOrig, const Rect& theSrcRectOrig,
                               const Rect& theClipRect, const Color& theColor, int theDrawMode, bool fastStretch)
{
    theImage->mDrawn = true;
...
void DDImage::BltMatrix(Image* theImage, float x, float y, const SexyMatrix3& theMatrix, const Rect& theClipRect,
                        const Color& theColor, int theDrawMode, const Rect& theSrcRect, bool blend)
{
    ...
    if (Check3D(this))
    {
        mDDInterface->mD3DInterface->BltTransformed(theImage, &theClipRect, theColor, theDrawMode, theSrcRect,
    ...
    BltMatrixHelper(theImage, x, y, theMatrix, theClipRect, theColor, theDrawMode, theSrcRect,
                    mLockedSurfaceDesc.lpSurface, mLockedSurfaceDesc.lPitch, aPixelFormat, blend);
...
void DDImage::BltTrianglesTex(Image* theTexture, const TriVertex theVertices[][3], int theNumTriangles,
                              ...
                              bool blend)
{
    ...
    if (Check3D(this))
    {
        mDDInterface->mD3DInterface->DrawTrianglesTex(theVertices, theNumTriangles, theColor, theDrawMode, theTexture,
                                                      tx, ty, blend);
        return;
    }
    ...
    BltTrianglesTexHelper(theTexture, theVertices, theNumTriangles, theClipRect, theColor, theDrawMode,
                          mLockedSurfaceDesc.lpSurface, mLockedSurfaceDesc.lPitch, aPixelFormat, tx, ty, blend);
    UnlockSurface();
...
void DDImage::FillScanLinesWithCoverage(Span* theSpans, int theSpanCount, const Color& theColor, int theDrawMode,
