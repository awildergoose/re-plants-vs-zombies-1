...
SysFont::SysFont(SexyAppBase* theApp, const std::string& theFace, int thePointSize, int theScript, bool bold,
                   ...
                   bool italics, bool underline, bool useDevCaps)
{
    ...
    mHFont =
        CreateFontA(aHeight, 0, 0, 0, bold ? FW_BOLD : FW_NORMAL, italics, underline, false, theScript, OUT_TT_PRECIS,
...
void SysFont::DrawString(Graphics* g, int theX, int theY, const SexyString& theString, const Color& theColor,
    ...
    if (aDDImage != NULL)
    {
        ...
        if (aSurface != NULL)
        {
            ...
            if (aSurface->GetDC(&aDC) == DD_OK)
            {
                HFONT anOldFont = (HFONT)SelectObject(aDC, mHFont);
                SetBkMode(aDC, TRANSPARENT);
                IntersectClipRect(aDC, theClipRect.mX, theClipRect.mY, theClipRect.mX + theClipRect.mWidth,
                                  theClipRect.mY + theClipRect.mHeight);

                if (mDrawShadow)
                {
                    SetTextColor(aDC, RGB(0, 0, 0));
                    TextOut(aDC, theX + g->mTransX + 1, theY - mAscent + 1 + g->mTransY + 1, theString.c_str(),
                            theString.length());
                    if (mSimulateBold)
                        TextOut(aDC, theX + g->mTransX + 2, theY - mAscent + 1 + g->mTransY + 1, theString.c_str(),
                                theString.length());
                }
                ...
                if (mSimulateBold)
                    TextOut(aDC, theX + g->mTransX + 1, theY - mAscent + 1 + g->mTransY, theString.c_str(),
                            theString.length());
            ...
            if (aDDImage->mLockCount > 0)
                aDDImage->mSurface->Lock(NULL, &aDDImage->mLockedSurfaceDesc, DDLOCK_SURFACEMEMORYPTR | DDLOCK_WAIT,
                                         NULL);
        }
    }
