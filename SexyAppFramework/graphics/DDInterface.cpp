...
DDInterface::DDInterface(SexyAppBase* theApp)
{
    ...
    gDirectDrawCreateFunc =
        reinterpret_cast<DirectDrawCreateFunc>(reinterpret_cast<void*>(GetProcAddress(gDDrawDLL, "DirectDrawCreate")));
    gDirectDrawCreateExFunc = reinterpret_cast<DirectDrawCreateExFunc>(
        reinterpret_cast<void*>(GetProcAddress(gDDrawDLL, "DirectDrawCreateEx")));
}
...
int DDInterface::Init(HWND theWindow, bool IsWindowed)
{
    ...
    OutputDebug(_S("Application requests %4lu x %4lu [%2d:%2d]\n"), mWidth, mHeight, mAspect.mNumerator,
                mAspect.mDenominator);
    ...
    if (IsWindowed)
    {
        ...
        if (mApp->mEnableWindowAspect && mAspect < mApp->mWindowAspect)
        {
            ...
            ::MoveWindow(theWindow, std::max((LONG)0, rc.left), std::max((LONG)0, rc.top), rc.right - rc.left,
    ...
    if (mPrimarySurface != NULL)
    {
        ...
        for (i = 0; i < aMaxR * 2 + 1; i++) mRedAddTable[i] = std::min(i, aMaxR);
        for (i = 0; i < aMaxG * 2 + 1; i++) mGreenAddTable[i] = std::min(i, aMaxG);
        for (i = 0; i < aMaxB * 2 + 1; i++) mBlueAddTable[i] = std::min(i, aMaxB);
...
bool DDInterface::CopyBitmap(LPDIRECTDRAWSURFACE theSurface, HBITMAP theBitmap, int theX, int theY,
                             unsigned int theWidth, unsigned int theHeight)
{
    ...
    if (hdcImage != NULL)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            {
                StretchBlt(hdc, 0, 0, aDesc.dwWidth, aDesc.dwHeight, hdcImage, theX, theY, theWidth, theHeight,
                           SRCCOPY);
            }
...
void DDInterface::RestoreOldCursorAreaFrom(LPDIRECTDRAWSURFACE theSurface, bool adjust)
{
    ...
    {
        Rect aSexyScreenRect(mCursorX - (mCursorWidth / 2), mCursorY - (mCursorHeight / 2), mCursorWidth,
                             mCursorHeight);

        Rect aClippedScreenRect = aSexyScreenRect.Intersection(Rect(0, 0, mWidth, mHeight));

        Rect aSexyLocalRect(aClippedScreenRect.mX - aSexyScreenRect.mX, aClippedScreenRect.mY - aSexyScreenRect.mY,
...
void DDInterface::DrawCursorTo(LPDIRECTDRAWSURFACE theSurface, bool adjust)
{
    if ((mCursorImage != NULL) && (mPrimarySurface != NULL))
    {
        ...
        if (theSurface == mDrawSurface && !mIs3D)
        {
            ...
            g.DrawImage(mCursorImage, mCursorX - (mCursorWidth / 2) + (mCursorWidth - mCursorImage->mWidth) / 2,
        ...
        else
        {
            ...
            aNewCursorAreaG.DrawImage(mCursorImage, (mCursorWidth - mCursorImage->mWidth) / 2,
...
void DDInterface::MoveCursorTo(LPDIRECTDRAWSURFACE theSurface, bool adjust, int theNewCursorX, int theNewCursorY)
{
    ...
    if ((mCursorImage != NULL) && (mPrimarySurface != NULL))
    {
        ...
        // NEW RECTANGLES
        Rect aNewSexyScreenRect(theNewCursorX - (mCursorWidth / 2), theNewCursorY - (mCursorHeight / 2), mCursorWidth,
                                mCursorHeight);
        ...
        Rect aNewSexyLocalRect(aNewClippedScreenRect.mX - aNewSexyScreenRect.mX,
                               aNewClippedScreenRect.mY - aNewSexyScreenRect.mY, aNewClippedScreenRect.mWidth,
        ...
        if ((aSexySrcAreaRect.mWidth > 0) || (aSexySrcAreaRect.mHeight > 0))
        {
            Rect aSexyDestAreaRect(aSexySrcAreaRect.mX - aSexyOrigSrcAreaRect.mX,
                                   aSexySrcAreaRect.mY - aSexyOrigSrcAreaRect.mY, aSexySrcAreaRect.mWidth,
