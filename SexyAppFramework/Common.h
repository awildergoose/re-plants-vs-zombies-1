...
template <typename T>
T SexyGetProc(HMODULE module, const char* name)
{
...
inline BOOL GetVersionExModern(OSVERSIONINFOA* lpVersionInfo)
{
    ...
    auto func = SexyGetProc<RtlGetVersionPtr>(ntdll, "RtlGetVersion");
    if (!func) return FALSE;

    RTL_OSVERSIONINFOW versionW  = {};
    versionW.dwOSVersionInfoSize = sizeof(versionW);

    if (func(&versionW) != 0)  // nonzero = failure
        return FALSE;

    // Copy to OSVERSIONINFOA
    lpVersionInfo->dwMajorVersion = versionW.dwMajorVersion;
    lpVersionInfo->dwMinorVersion = versionW.dwMinorVersion;
    lpVersionInfo->dwBuildNumber  = versionW.dwBuildNumber;
    lpVersionInfo->dwPlatformId   = versionW.dwPlatformId;

    // Convert wide string to ANSI
    size_t copied;
    wcstombs_s(&copied, lpVersionInfo->szCSDVersion, versionW.szCSDVersion, _TRUNCATE);
