...
DSoundManager::DSoundManager(HWND theHWnd, bool haveFMod)
{
    ...
    for (i = 0; i < MAX_CHANNELS; i++) mPlayingSounds[i] = NULL;
    ...
    if (theHWnd != NULL)
    {
        ...
        DirectSoundCreateFunc aDirectSoundCreateFunc = reinterpret_cast<DirectSoundCreateFunc>(
            reinterpret_cast<void*>(GetProcAddress(gDSoundDLL, "DirectSoundCreate")));
...
bool DSoundManager::LoadWAVSound(unsigned int theSfxID, const std::string& theFilename)
{
    ...
    while (!p_feof(fp))
    {
        ...
        else if (strcmp(aChunkType, "data") == 0)
        {
            ...
            for (int i = 0; i < aDataSize; i++) ((uchar*)lpvPtr)[i] ^= anXor;
...
int ov_pak_open(PFILE* f, OggVorbis_File* vf, char* initial, long ibytes)
{
    ov_callbacks callbacks = {
        (reinterpret_cast<size_t (*)(void*, size_t, size_t, void*)>(reinterpret_cast<void*>(p_fread))),
        (int (*)(void*, ogg_int64_t, int))p_fseek64_wrap, (int (*)(void*))p_fclose, (long (*)(void*))p_ftell};
...
bool DSoundManager::WriteWAV(unsigned int theSfxID, const std::string& theFilename, const std::string& theDepFile)
{
    ...
    for (DWORD i = 0; i < dwBytes; i++) ((uchar*)lpvPtr)[i] ^= anXor;
