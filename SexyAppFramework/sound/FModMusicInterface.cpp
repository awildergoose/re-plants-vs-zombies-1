...
bool FModMusicInterface::LoadSample(int theSongId, const std::string& theIntroFileName,
                                    const std::string& theRepeatFileName, bool repeat)
{
    ...
    FSOUND_SAMPLE* aSample = gFMod->FSOUND_Sample_Alloc(FSOUND_FREE, anIntroLen + aLoopLen,
                                                        anIntroMode | FSOUND_LOOP_NORMAL, anIntroFreq, 255, 128, 0);
    ...
    if (gFMod->FSOUND_Sample_Lock(aSample, 0, (anIntroLen + aLoopLen) * aSampleBytes, &aSamplePtr1, &aSamplePtr2,
                                  &aSampleLen1, &aSampleLen2))
    {
        void* aSrcPtr1;
        void* aSrcPtr2;
        uint  aSrcLen1;
        ...
        if (gFMod->FSOUND_Sample_Lock(anIntroSample, 0, anIntroLen * aSampleBytes, &aSrcPtr1, &aSrcPtr2, &aSrcLen1,
                                      &aSrcLen2))
        {
        ...
        if (gFMod->FSOUND_Sample_Lock(aLoopSample, anIntroLen * aSampleBytes, aLoopLen * aSampleBytes, &aSrcPtr1,
                                      &aSrcPtr2, &aSrcLen1, &aSrcLen2))
        {
...
void FModMusicInterface::PlayMusic(int theSongId, int theOffset, bool noLoop)
{
    ...
    if (anItr != mMusicMap.end())
    {
        ...
        {
            gFMod->FMUSIC_SetMasterVolume(aMusicInfo->mHMusic,
                                          (int)(mMasterVolume * aMusicInfo->mVolume * mMaxMusicVolume));
