...
TodStringListFormat::TodStringListFormat(const char* theFormatName, _Font** theFont, const Color& theColor,
                                         int theLineSpacingOffset, unsigned int theFormatFlags)
    : mFormatName(theFormatName),
      mNewFont(theFont),
      mNewColor(theColor),
      mLineSpacingOffset(theLineSpacingOffset),
...
// 0x519870
int TodWriteString(Graphics* g, const SexyString& theString, int theX, int theY, TodStringListFormat& theCurrentFormat,
                   int theWidth, DrawStringJustification theJustification, bool drawString, int theOffset,
                   int theLength)
{
    _Font* aFont = *theCurrentFormat.mNewFont;
    if (drawString)  // 如果需要实际绘制
    {
        int aSpareX = theWidth - TodWriteString(g, theString, theX, theY, theCurrentFormat, theWidth,
                              ...
                              int theMaxChars)
{
    ...
    return TodWriteString(g, theString, theX, theY, theCurrentFormat, theWidth, theJustification, drawString, theOffset,
                          theLength);
}

// 0x519B50
//  GOTY @Patoke: 0x5241C0
int TodDrawStringWrappedHelper(Graphics* g, const SexyString& theText, const Rect& theRect, _Font* theFont,
                               const Color& theColor, DrawStringJustification theJustification, bool drawString)
{
    ...
    while (aCurPos < theText.size())
    {
        ...
        if (aCurWidth > theRect.mWidth)  // 如果当前宽度超出了限制区域的宽度，则进行换行的处理
        {
            ...
            else
            {
                ...
                aLineWidth =
                    TodWriteWordWrappedHelper(g, theText, theRect.mX, theRect.mY + aYOffset, aCurrentFormat,
                                              theRect.mWidth, theJustification, drawString,
                                              aLineFeedPos,            // 上次换行的位置即为新行开始的位置
...
void TodDrawStringWrapped(Graphics* g, const SexyString& theText, const Rect& theRect, _Font* theFont,
                          const Color& theColor, DrawStringJustification theJustification)
{
    ...
    {
        aRectTodUse.mY += (aRectTodUse.mHeight - TodDrawStringWrappedHelper(g, aTextFinal, aRectTodUse, theFont,
