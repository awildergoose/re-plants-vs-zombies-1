...
DefMap   gParticleDefMap      = {gParticleDefFields, sizeof(TodParticleDefinition),
                                 TodParticleDefinitionConstructor};  // 0x69E690
...
DefMap   gReanimatorTransformDefMap      = {gReanimatorTransformDefFields, sizeof(ReanimatorTransform),
                                            ReanimatorTransformConstructor};  // 0x69F07C
...
DefMap   gReanimatorTrackDefMap      = {gReanimatorTrackDefFields, sizeof(ReanimatorTrack),
                                        ReanimatorTrackConstructor};  // 0x69F178
...
DefMap gReanimatorDefMap = {gReanimatorDefFields, sizeof(ReanimatorDefinition),
                            ReanimatorDefinitionConstructor};  // 0x69F1B4

static DefLoadResPath gDefLoadResPaths[4] = {{"IMAGE_", ""},
                                             {"IMAGE_", "particles\\"},
                                             {"IMAGE_REANIM_", "reanim\\"},
                                             {"IMAGE_REANIM_", "images\\"}};  // 0x6A1A48
...
inline bool DefReadFromCacheArray(void*& theReadPtr, DefinitionArrayDef* theArray, DefMap* theDefMap)
{
    ...
    SMemR(theReadPtr, theArray->mArrayData,
    ...
    for (int i = 0; i < theArray->mArrayCount; i++)
        if (!DefMapReadFromCache(
                ...
                (void*)((intptr_t)theArray->mArrayData + theDefMap->mDefSize * i)))  // 最后一个参数表示 pData[i]
            return false;
    return true;
}
                                         ...
                                         size_t& theUncompressedSize, const SexyString& theCompiledFilePath)
{
    ...
    Bytef* aSrc                = (Bytef*)((intptr_t)theCompressedBuffer +
                           sizeof(CompressedDefinitionHeader));  // 实际解压数据从第 3 个四字节开始
    // BuGFIXX!!
    ulong aUncompressedSizeResult = aHeader->mUncompressedSize;  // 用作出参的未压缩数据实际长度
    int   aResult                 = uncompress(aUncompressedBuffer, &aUncompressedSizeResult, aSrc,
...
bool IsFileInPakFile(const SexyString& theFilePath)
{
    ...
    bool   aIsInPak =
        pFile &&
        !pFile->mFP;  // 通过 mPakRecordMap.find 找到并打开的文件，其 mFP 为空指针（因为不是从实际文件中打开的）
    if (pFile)
    {
        p_fclose(pFile);
    }
...
bool DefSymbolValueFromString(DefSymbol* theSymbolMap, const char* theName, int* theResultValue)
{
    while (theSymbolMap->mSymbolName != nullptr)
    {
        if (_stricmp(theName, theSymbolMap->mSymbolName) == 0)
        {
...
bool DefinitionReadFlagField(XMLParser* theXmlParser, const SexyString& theElementName, uint* theResultValue,
...
bool DefinitionReadImageField(XMLParser* theXmlParser, Image** theImage)
{
    ...
    std::string aMessgae = StrFormat("Failed to find image '%s' in %s", SexyStringToStringFast(aStringValue).c_str(),
...
bool DefinitionReadFontField(XMLParser* theXmlParser, _Font** theFont)
{
    ...
    std::string aMessgae = StrFormat("Failed to find font '%s' in %s", SexyStringToStringFast(aStringValue).c_str(),
                                     theXmlParser->GetFileName().c_str());
...
bool DefinitionReadField(XMLParser* theXmlParser, DefMap* theDefMap, void* theDefinition, bool* theDone)
{
    ...
    for (DefField* aField = theDefMap->mMapFields; *aField->mFieldName != '\0'; aField++)
    {
        ...
        if (_stricmp(aXMLElement.mValue.c_str(), aField->mFieldName) == 0)  // 判断 aXMLElement 定义的是否为该成员变量
        {
            bool aSuccess;
            switch (aField->mFieldType)
            {
    ...
    DefinitionXmlError(theXmlParser, "Ignoring unknown element '%s'",
                       aXMLElement.mValue.c_str());  // aXMLElement 未定义任何成员变量时
    return false;
}
...
void* DefinitionCompressCompiledBuffer(void* theBuffer, unsigned int theBufferSize, unsigned int* theResultSize)
{
    ...
    compress((Bytef*)((uintptr_t)aCompressedBuffer + sizeof(CompressedDefinitionHeader)), &aCompressedSize,
...
void DefinitionFreeArrayField(DefinitionArrayDef* theArray, DefMap* theDefMap)
{
    for (int i = 0; i < theArray->mArrayCount; i++)
        DefinitionFreeMap(
            theDefMap, (void*)((intptr_t)theArray->mArrayData + theDefMap->mDefSize * i));  // 最后一个参数表示 pData[i]
...
void DefinitionFreeMap(DefMap* theDefMap, void* theDefinition)
{
    ...
    for (DefField* aField = theDefMap->mMapFields; *aField->mFieldName != '\0'; aField++)
    {
        ...
        {
            case DefFieldType::DT_STRING:
                // @Patoke todo: removed this, caused a heap problem when closing the game, add back properly (causes
                // memory leak)
                // if (**(char**)aVar != '\0')
