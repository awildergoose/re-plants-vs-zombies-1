...
ReanimationParams gLawnReanimationArray[(int)ReanimationType::NUM_REANIMS] = {
    ...
    {ReanimationType::REANIM_BACKUP_DANCER, "reanim\\Zombie_backup.reanim",
     0},  // @Patoke: GOTY has different reanim name
...
void ReanimationCreateAtlas(ReanimatorDefinition* theDefinition, ReanimationType theReanimationType)
{
    ...
    if (aDuration > 20 && theReanimationType != ReanimationType::REANIM_NONE)  // （仅内测版）创建时间过长的报告
        TodTraceAndLog("LOADING:Long atlas '%s' %d ms on %s", aParam.mReanimFileName, aDuration,
...
void Reanimation::Update()
{
    ...
    for (int aTrackIndex = 0; aTrackIndex < mDefinition->mTracks.count; aTrackIndex++)
    {
        ...
        if (_strnicmp(mDefinition->mTracks.tracks[aTrackIndex].mName, "attacher__", 10) == 0)  // IsAttacher
                    ...
                    const ReanimatorTransform& theTransform2, float theBlendFactor)
{
    ...
    while (aSkewX2 > theTransform1.mSkewX + 180.0f) aSkewX2 = theTransform1.mSkewX;  // （aSkewX2 -= 360.0f）
    while (aSkewX2 < theTransform1.mSkewX - 180.0f) aSkewX2 = theTransform1.mSkewX;  // （aSkewX2 += 360.0f）
    while (aSkewY2 > theTransform1.mSkewY + 180.0f) aSkewY2 = theTransform1.mSkewY;  // （aSkewY2 -= 360.0f）
    while (aSkewY2 < theTransform1.mSkewY - 180.0f) aSkewY2 = theTransform1.mSkewY;  // （aSkewY2 += 360.0f）
...
// 0x472190
void Reanimation::ReanimBltMatrix(Graphics* g, Image* theImage, SexyMatrix3& theTransform, const Rect& theClipRect,
...
bool Reanimation::DrawTrack(Graphics* g, int theTrackIndex, int theRenderGroup, TodTriangleGroup* theTriangleGroup)
{
    ...
    {
        if (_stricmp(mDefinition->mTracks.tracks[theTrackIndex].mName,
    ...
    if (aAtlasImage != nullptr)  // 如果存在图集（动画定义存在 atlas，轨道变换存在图像，轨道不存在覆写贴图）
    {
        ...
        if (mEnableExtraOverlayDraw)
        {
            theTriangleGroup->AddTriangle(g, FilterEffectGetImage(aImage, FilterEffect::FILTER_EFFECT_WHITE), aMatrix,
    ...
    else if (aImage != nullptr)  // 如果不存在 atlas 但轨道变换存在图像
    {
        ...
        if (mEnableExtraOverlayDraw)
        {
            ...
            ReanimBltMatrix(g, aOverlayImage, aMatrix, aClipRect, aExtraOverlayColor, Graphics::DRAWMODE_NORMAL,
                            aSrcRect);
        }
    }
...
void Reanimation::GetTrackMatrix(int theTrackIndex, SexyTransform2D& theMatrix)
{
    ...
    SexyMatrix3Translation(theMatrix, aTrackInstance->mShakeX - 0.5f,
                           aTrackInstance->mShakeY - 0.5f);  // 轨道震动的影响
}
...
{
    for (int aTrackIndex = 0; aTrackIndex < mDefinition->mTracks.count; aTrackIndex++)
        if (_stricmp(mDefinition->mTracks.tracks[aTrackIndex].mName, theTrackName) == 0) return aTrackIndex;
...
// 0x473070
AttachEffect* Reanimation::AttachParticleToTrack(const char* theTrackName, TodParticleSystem* theParticleSystem,
...
{
    for (int aTrackIndex = 0; aTrackIndex < mDefinition->mTracks.count; aTrackIndex++)
        if (_stricmp(mDefinition->mTracks.tracks[aTrackIndex].mName, theTrackName) == 0) return true;
    return false;
}
...
void ReanimatorEnsureDefinitionLoaded(ReanimationType theReanimType, bool theIsPreloading)
{
    ...
    else  // < 以下部分仅内测版执行 >
    {
        if (gAppHasUsedCheatKeys())
            TodTraceAndLog("Cheater failed to preload '%s' on %s", aReanimParams->mReanimFileName,
                           gGetCurrentLevelName().c_str());
        else
            TodTraceAndLog("Non-cheater failed to preload '%s' on %s", aReanimParams->mReanimFileName,
                           gGetCurrentLevelName().c_str());
    }  // < 以上部分仅内测版执行 >
    ...
    if (aDuration > 100)  // （仅内测版）创建时间过长的报告
        TodTraceAndLog("LOADING:Long reanim '%s' %d ms on %s", aReanimParams->mReanimFileName, aDuration,
                       gGetCurrentLevelName().c_str());
}
...
{
    for (unsigned int i = 0; i < gReanimatorDefCount; i++) ReanimationFreeDefinition(&gReanimatorDefArray[i]);
...
void Reanimation::ShowOnlyTrack(const char* theTrackName)
{
    for (int i = 0; i < mDefinition->mTracks.count; i++)
    {
        ...
        mTrackInstances[i].mRenderGroup = _stricmp(mDefinition->mTracks.tracks[i].mName, theTrackName) == 0
                                              ? RENDER_GROUP_NORMAL
                                              : RENDER_GROUP_HIDDEN;
    }
}
...
{
    for (int i = 0; i < mDefinition->mTracks.count; i++)
        if (_stricmp(mDefinition->mTracks.tracks[i].mName, theTrackName) == 0)
        {
...
void Reanimation::AssignRenderGroupToPrefix(const char* theTrackName, int theRenderGroup)
{
    ...
    for (int i = 0; i < mDefinition->mTracks.count; i++)
    {
        ...
        if (strlen(aTrackName) >= aPrifixLength &&
            !_strnicmp(aTrackName, theTrackName, aPrifixLength))  // 轨道名称长度必须不小于指定前缀长度
...
{
    for (int i = 0; i < mDefinition->mTracks.count; i++)
        AttachmentPropogateColor(mTrackInstances[i].mAttachmentID, mColorOverride, mEnableExtraAdditiveDraw,
...
void Reanimation::UpdateAttacherTrack(int theTrackIndex)
{
    ...
    if (aAttacherInfo.mReanimName.size() != 0)  // 如果附属轨道设定了当前的附属动画名称
    {
        ...
        for (unsigned int i = 0; i < gReanimationParamArraySize; i++)  // 在动画参数数组中寻找动画文件名对应的动画类型
        {
            ...
            if (_stricmp(aReanimFileName.c_str(), aParams->mReanimFileName) == 0)
            {
    ...
    AttachmentPropogateColor(aTrackInstance->mAttachmentID, aColor, mEnableExtraAdditiveDraw, mExtraAdditiveColor,
