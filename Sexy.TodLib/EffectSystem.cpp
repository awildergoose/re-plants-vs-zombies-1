...
// 0x446100
static inline int Tod_clipShape(TriVertex** dst, TriVertex* src, const float left, const float right, const float top,
                                const float bottom)
{
...
{
    for (int i = 0; i < 256; i++)
        for (int j = 0; j < 3; j++) mVertArray[i][j].color = 0UL;
...
// 0x446300
void TodTriangleGroup::AddTriangle(Graphics* g, Image* theImage, const SexyMatrix3& theMatrix, const Rect& theClipRect,
