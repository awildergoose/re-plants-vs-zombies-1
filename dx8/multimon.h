...
#define xPRIMARY_MONITOR ((HMONITOR)0x12340042)
                                     ...
                                     MONITORENUMPROC lpfnEnumProc, LPARAM dwData)
    {
        ...
        if (hdcOptionalForPainting)
        {
            ...
            {
                default:
                    if (!GetDCOrgEx(hdcOptionalForPainting, &ptOrg)) return FALSE;
    ...
    {
        if (InitMultipleMonitorStubs()) return g_pfnEnumDisplayDevices(Unused, iDevNum, lpDisplayDevice, dwFlags);

        if (Unused != NULL) return FALSE;

        if (iDevNum != 0) return FALSE;

        if (lpDisplayDevice == NULL || lpDisplayDevice->cb < sizeof(DISPLAY_DEVICE)) return FALSE;
