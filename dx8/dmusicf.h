...
#define DMUS_FOURCC_WAVEHEADER_CHUNK mmioFOURCC('w', 'a', 'v', 'h')
    ...
    {
        REFERENCE_TIME rtReadAhead; /* How far ahead in the stream wave data will be read (in REFERENCE_TIME).  Ignored
                                       for one-shot waves.  */
        DWORD
...
#define DMUS_WAVETRACKF_PERSIST_CONTROL 0x2
    ...
    {
        long lVolume; /* Gain, in 1/100th of dB, to be applied to all waves in wave part.  Note:  All gain values should
                         be negative. */
    ...
    typedef struct _DMUS_IO_WAVE_ITEM_HEADER
    {
        ...
        MUSIC_TIME mtLogicalTime; /* If in music track format, this indicates the musical boundary where this belongs.
                                     Otherwise, ignored. */
...
#define DMUS_SEGIOF_REFLENGTH 1 /* Use the time in rtLength for the segment length. */
    ...
    (
        ...
        (
            ...
            [<guid-ck>]     // Optional GUID for track object instance (not to be confused with Class id in track
    header)
            [<vers-ck>]     // Optional version info
...
#define DMUS_FOURCC_SONGSEGMENT_LIST                                                                                \
    mmioFOURCC('s', 's', 'g', 'l')                                 /* Container for a segment or segment reference. \
                                                                    */
...
#define DMUS_SONG_NOFROMSEG                                                                                      \
    0x80000001 /* Special ID for dwSegmentID to indicate transition from nothing (or outside the song) into this \
                  segment. */
...
#define DMUS_SCRIPTIOF_DOWNLOAD_ALL_SEGMENTS (1 << 1)
    /* If set and LOAD_ALL_CONTENT is also set, when the script initializes it will also download all the segments in
       its container. If set and LOAD_ALL_CONTENT is not set, when the script calls segment.Load on a segment then the
       segment will also be downloaded. If not set, the script must manually download and unload by calling
       segment.DownloadSoundData and segment.UnloadSoundData. */
...
#define DMUS_IO_SCRIPTTRACKF_PREPARE                                                                             \
    (1 << 0) /* Fire event in advance of time stamp, at Prepare time.  This is the default because it leaves the \
                script time to change the music happening at the target time. */
...
#define DMUS_FOURCC_LYRICSTRACKEVENTTEXT_CHUNK mmioFOURCC('l', 'y', 'r', 'n')
    ...
    {
        DWORD dwFlags;            /* Reserved leave as 0. */
        DWORD dwTimingFlags;      /* Combination DMUS_PMSGF_TOOL_* flags.  Determines the precise timing of when the
                                     notification happens. Invalid with the flag DMUS_PMSGF_REFTIME, DMUS_PMSGF_MUSICTIME,
...
#define DMUS_FOURCC_PARAMCONTROLTRACK_CURVES_CHUNK mmioFOURCC('p', 'r', 'c', 'c')
    ...
    typedef struct _DMUS_IO_PARAMCONTROLTRACK_OBJECTHEADER
    {
        ...
        GUID  guidTimeFormat; /* Time format to set the object to.  Must be GUID_TIME_REFERNCE or GUID_TIME_MUSIC from
                                 medparam.h. */
        /* Path for finding the object. These fields correspond to the first five parameters of
         * IDirectMusicSegmentState::GetObjectInPath. */
        DWORD dwPChannel;
        DWORD dwStage;
        DWORD dwBuffer;
        GUID  guidObject;
