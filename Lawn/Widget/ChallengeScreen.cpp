...
ChallengeDefinition gChallengeDefs[NUM_CHALLENGE_MODES] = {
    ...
    {GameMode::GAMEMODE_TREE_OF_WISDOM, 10, ChallengePage::CHALLENGE_PAGE_LIMBO, 2, 4,
     _S("Tree of Wisdom")},  // @Patoke: replaced for english
...
ChallengeScreen::ChallengeScreen(LawnApp* theApp, ChallengePage thePage)
{
    ...
    mBackButton = MakeNewButton(ChallengeScreen::ChallengeScreen_Back, this, _S("[BACK_TO_MENU]"), nullptr,
...
void ChallengeScreen::DrawButton(Graphics* g, int theChallengeIndex)
{
    ...
    if (aChallengeButton->mVisible)
    {
        ...
        if (AccomplishmentsNeeded(theChallengeIndex) <= 1)
        {
            ...
            g->DrawImage(aHighLight ? Sexy::IMAGE_CHALLENGE_WINDOW : Sexy::IMAGE_CHALLENGE_WINDOW_HIGHLIGHT, aPosX - 6,
                         aPosY - 2);
            ...
            else
            {
                ...
                // 分别绘制两行文本字符串
                TodDrawString(g, aName.substr(0, aLine1Len), aPosX + 52, aPosY + 88, Sexy::FONT_BRIANNETOD12,
                              aTextColor, DS_ALIGN_CENTER);
                if (aLine2Len > 0)
                {
                    TodDrawString(g, aName.substr(aLine1Len + 1, aLine2Len), aPosX + 52, aPosY + 102,
                                  Sexy::FONT_BRIANNETOD12, aTextColor, DS_ALIGN_CENTER);
                }
            }
...
void ChallengeScreen::Draw(Graphics* g)
{
    ...
    for (int aChallengeMode = 0; aChallengeMode < NUM_CHALLENGE_MODES; aChallengeMode++) DrawButton(g, aChallengeMode);
