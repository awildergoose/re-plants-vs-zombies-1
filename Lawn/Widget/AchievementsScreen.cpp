...
void AchievementsWidget::Draw(Graphics* g)
{
    ...
    for (int i = 1; i <= 70; i++) g->DrawImage(IMAGE_ACHEESEMENTS_HOLE_TILE, 0, aHeight * i);
    ...
    for (int i = 0; i < MAX_ACHIEVEMENTS; i++)
    {
        ...
        if (mApp->mPlayerInfo)
            aHasAchievement = mApp->mPlayerInfo->mEarnedAchievements[i];
        else
            aHasAchievement = false;
    ...
    {
        g->DrawImage(aIsHighlight ? IMAGE_ACHEESEMENTS_MORE_BUTTON_HIGHLIGHT : IMAGE_ACHEESEMENTS_MORE_BUTTON, 700,
                     450);
    }
}
