                             ...
                             const SexyString& theMessage)
{
    ...
    TodDrawStringWrapped(g, theMessage, Rect(285, 360, 230, 90), Sexy::FONT_BRIANNETOD16, Color(40, 50, 90),
...
void AwardScreen::Draw(Graphics* g)
{
    ...
    else if (mAwardType != AWARD_ACHIEVEMENTONLY)  // @Patoke: add check
    {
        ...
        else if (aLevel == 10)
        {
            ...
            TodDrawString(g, _S("[FOUND_NOTE]"), BOARD_WIDTH / 2, 70, Sexy::FONT_DWARVENTODCRAFT24,
                          Color(255, 200, 0, 255), DS_ALIGN_CENTER);
        }
        else if (aLevel == 15)
        {
            DrawBottom(g, _S("[FOUND_SUBURBAN_ALMANAC]"), _S("[SUBURBAN_ALMANAC]"),
                       _S("[SUBURBAN_ALMANAC_DESCRIPTION]"));
...
void AwardScreen::DrawAchievements(Graphics* g)
{
    ...
    for (size_t i = 0; i < mAchievementItems.size(); i++)
    {
        ...
        TodDrawString(g, aAchievementName, BOARD_WIDTH / 2, mAchievementItems[i].mY + 25, FONT_DWARVENTODCRAFT15,
                      Color(224, 187, 98), DS_ALIGN_CENTER);
        TodDrawStringWrapped(g, aAchievementName, aTextRect, FONT_DWARVENTODCRAFT12, Color(255, 255, 255),
                             DS_ALIGN_CENTER_VERTICAL_MIDDLE);
    }
}
