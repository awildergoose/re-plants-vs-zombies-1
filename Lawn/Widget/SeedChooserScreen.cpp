...
{
    int aTotalWeight = 0;
    for (int i = 0; i < theCount; i++) aTotalWeight += theArray[i].mWeight;
...
void SeedChooserScreen::Draw(Graphics* g)
{
    ...
    TodDrawString(g, _S("[CHOOSE_YOUR_PLANTS]"), 229, 110, Sexy::FONT_DWARVENTODCRAFT18YELLOW, Color::White,
                  DS_ALIGN_CENTER);
...
{
    return mApp->LawnMessageBox(Dialogs::DIALOG_CHOOSER_WARNING, _S("[DIALOG_WARNING]"), theMessage,
                                _S("[DIALOG_BUTTON_YES]"), _S("[REPICK_BUTTON]"),
...
void SeedChooserScreen::MouseDown(int x, int y, int theClickCount)
{
    ...
    else
    {
        ...
        if (aSeedType != SEED_NONE && !SeedNotAllowedToPick(aSeedType))
        {
            ...
            {
                mApp->PlaySample(Sexy::SOUND_TAP);
                if (mApp->LawnMessageBox(DIALOG_MESSAGE, _S("[GET_FULL_VERSION_TITLE]"), _S("[GET_FULL_VERSION_BODY]"),
                                         _S("[GET_FULL_VERSION_YES_BUTTON]"), _S("[GET_FULL_VERSION_NO_BUTTON]"),
                                         Dialog::BUTTONS_YES_NO) == Dialog::ID_YES)
                {
                    if (mApp->mDRM)
                    {
