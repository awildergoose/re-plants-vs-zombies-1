...
void CutScene::AdvanceCrazyDaveDialog(bool theJustSkipping)
{
    ...
    if ((aMessageIndex == 1503 || aMessageIndex == 1553) && !theJustSkipping)
    {
        ...
        // 创建询问是否升级卡槽格数的对话
        Dialog* aDialog = mApp->DoDialog(Dialogs::DIALOG_PURCHASE_PACKET_SLOT, true, aAmountString, aBodyString, _S(""),
...
void CutScene::KeyDown(KeyCode theKey)
{
    if (mApp->mGameMode == GameMode::GAMEMODE_UPSELL)
    {
                 ...
                 theKey == KeyCode::KEYCODE_ESCAPE)
        {
            ...
            int aResult =
                mApp->LawnMessageBox(Dialogs::DIALOG_MESSAGE, _S("[UPSELL_PAUSE_HEADER]"), _S("[UPSELL_PAUSE_BODY]"),
    ...
    switch (mCrazyDaveLastTalkIndex)
    {
        case 3305:  // “像这个！”
        {
            ...
            AttachEffect* anAttachEffect =
                AttachReanim(aCrazyDaveReanim->GetTrackInstanceByName("Dave_handinghand")->mAttachmentID, aReanimSquash,
                             92.0f, 387.0f);
...
void CutScene::DrawIntro(Graphics* g)
{
    ...
    if (mCutsceneTime > TimeIntro_PresentsFadeIn && mCutsceneTime <= aTimePanRightStart)
    {
        ...
        TodDrawString(g, _S("[INTRO_PRESENTS]"), BOARD_WIDTH / 2 - mBoard->mX, 310 - mBoard->mY, FONT_BRIANNETOD16,
