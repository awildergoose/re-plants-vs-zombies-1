...
void Music::SetupMusicFileForTune(MusicFile theMusicFile, MusicTune theMusicTune)
{
    ...
    for (int aTrack = 0; aTrack < aTrackCount; aTrack++)
    {
        ...
        gBass->BASS_ChannelSetAttribute(aHMusic, BASS_ATTRIB_MUSIC_VOL_CHAN + aTrack,
                                        aVolume);  // 设置音乐每条轨道的音量属性（静音与否）
    }
}
...
void Music::LoadSong(MusicFile theMusicFile, const std::string& theFileName)
{
    ...
    {
        gBass->BASS_ChannelSetAttribute(GetBassMusicHandle(theMusicFile), BASS_ATTRIB_MUSIC_PSCALER,
                                        4);  // 设置音乐定位精确度属性
...
void Music::PlayFromOffset(MusicFile theMusicFile, int theOffset, double theVolume)
{
    ...
    else
    {
        ...
        gBass->BASS_ChannelSetPosition(aMusicInfo->mHMusic, MAKELONG(theOffset, 0) /**/,
                                       BASS_POS_MUSIC_ORDER);  // 设置偏移位置
...
void Music::UpdateMusicBurst()
{
    ...
    switch (mMusicBurstState)
    {
        ...
        case MusicBurstState::MUSIC_BURST_STARTING:
            ...
            {
                aFadeTrackVolume =
                    TodAnimateCurveFloat(400, 0, mBurstStateCounter, 0.0f, 1.0f, TodCurves::CURVE_LINEAR);
        ...
        case MusicBurstState::MUSIC_BURST_FINISHING:
            if (aBurstScheme == 1)
                aFadeTrackVolume =
                    TodAnimateCurveFloat(800, 0, mBurstStateCounter, 1.0f, 0.0f, TodCurves::CURVE_LINEAR);
            else
                aMainTrackVolume =
                    TodAnimateCurveFloat(400, 0, mBurstStateCounter, 0.0f, 1.0f, TodCurves::CURVE_LINEAR);
    ...
    else if (aBurstScheme == 2)
    {
        ...
        if (aDrumsJumpOrder != -1)
            gBass->BASS_ChannelSetPosition(GetBassMusicHandle(mCurMusicFileDrums),
                                           MAKELONG(aDrumsJumpOrder, 0) /*| 0x80000000*/, BASS_POS_MUSIC_ORDER);
    }
}
